"""Latent world model: CNN encoder + transition predictor in latent space.

Design choices:
- Encoder: small CNN that preserves spatial structure (much better than MLP for
  reasoning about walls and the agent on a grid).
- Predictor: MLP on (latent, action_embedding) -> next latent.
- Position head: small MLP that decodes the agent cell from the latent. This
  grounds the latent geometry; without it the JEPA-only objective leaves the
  latent too unconstrained for downstream planning.

The combination gives a learned simulator that is accurate enough to plan with
plain BFS in predicted-position space (see src/planner.py).
"""
from __future__ import annotations

import torch
import torch.nn as nn

from .data import GRID_SIZE, N_ACTIONS, N_CHANNELS

LATENT_DIM = 128
N_CELLS = GRID_SIZE * GRID_SIZE


class Encoder(nn.Module):
    def __init__(self, latent_dim: int = LATENT_DIM):
        super().__init__()
        self.conv = nn.Sequential(
            nn.Conv2d(N_CHANNELS, 32, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, 64, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(64, 64, kernel_size=3, padding=1),
            nn.ReLU(inplace=True),
        )
        self.head = nn.Sequential(
            nn.Flatten(),
            nn.Linear(64 * GRID_SIZE * GRID_SIZE, 256),
            nn.ReLU(inplace=True),
            nn.Linear(256, latent_dim),
        )

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        return self.head(self.conv(x))


class Predictor(nn.Module):
    def __init__(self, latent_dim: int = LATENT_DIM, n_actions: int = N_ACTIONS):
        super().__init__()
        self.action_emb = nn.Embedding(n_actions, 32)
        self.net = nn.Sequential(
            nn.Linear(latent_dim + 32, 256),
            nn.ReLU(inplace=True),
            nn.Linear(256, 256),
            nn.ReLU(inplace=True),
            nn.Linear(256, latent_dim),
        )

    def forward(self, z: torch.Tensor, a: torch.Tensor) -> torch.Tensor:
        emb = self.action_emb(a)
        return self.net(torch.cat([z, emb], dim=-1))


class PositionHead(nn.Module):
    def __init__(self, latent_dim: int = LATENT_DIM, n_cells: int = N_CELLS):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(latent_dim, 128),
            nn.ReLU(inplace=True),
            nn.Linear(128, n_cells),
        )

    def forward(self, z: torch.Tensor) -> torch.Tensor:
        return self.net(z)


class WorldModel(nn.Module):
    def __init__(self, latent_dim: int = LATENT_DIM):
        super().__init__()
        self.encoder = Encoder(latent_dim)
        self.predictor = Predictor(latent_dim)
        self.position_head = PositionHead(latent_dim)

    def encode(self, obs: torch.Tensor) -> torch.Tensor:
        return self.encoder(obs)

    def decode_pos(self, z: torch.Tensor) -> torch.Tensor:
        return self.position_head(z)

    def step(self, z: torch.Tensor, a: torch.Tensor) -> torch.Tensor:
        return self.predictor(z, a)

    def forward(self, obs_t: torch.Tensor, action: torch.Tensor, obs_tp1: torch.Tensor):
        z_t = self.encode(obs_t)
        z_tp1_pred = self.predictor(z_t, action)
        with torch.no_grad():
            z_tp1_target = self.encoder(obs_tp1)
        pos_logits_t = self.position_head(z_t)
        pos_logits_tp1 = self.position_head(z_tp1_pred)
        return z_t, z_tp1_pred, z_tp1_target, pos_logits_t, pos_logits_tp1


def count_params(model: nn.Module) -> int:
    return sum(p.numel() for p in model.parameters() if p.requires_grad)
